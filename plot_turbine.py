...
from plotting.plot_utils import parse_turbine_args, load_turbines

def main():
    # 1. Visual asthetics
    AcademicStyle.apply()

    cases = load_turbines(parse_turbine_args(), verbose=False)

    if len(cases) != 3:
        # p.plot_structure(cases, mode="xz")
        p.plot_spectrum(cases, do_thresholds=False, absorption=False)
        p.plot_polar(cases)
